...
def initial_omega(x,y):
    ...
    r = np.zeros(5)
    for i in range(10):
        r[0] = np.random.choice([-1,1])*np.random.uniform(0.5, 1)
        r[1] = np.random.uniform(-10,10)
        r[2] = np.random.choice([1,20])
        r[3] = np.random.uniform(-10,10)
        r[4] = np.random.choice([1,20])
        output += r[0]*np.exp(-(x-r[1])**2/r[2] - (y-r[3])**2/r[4])
    return output
...
imageio.mimsave("TenRandomVortices.gif", frames, duration=delta_t)
